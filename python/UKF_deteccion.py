...
def F4D4D_linearModel(x, u):
    ...
    return np.vstack([x[0, 0] + x[1, 0] * u[0, 0], x[1, 0], x[2, 0] + x[3, 0] * u[0, 0], x[3, 0]])
...
def H4D4D_identity(x):
    return np.vstack([x[0, 0], x[1, 0], x[2, 0], x[3, 0]])
...
xk0 = np.vstack([0.0, vals[0, 2] / vals[0, 0], 0.0, vals[0, 3] / vals[0, 0]])  # estado inicial propuesto
PK0 = np.eye(4)  # matriz de covarianza inicial
t0 = 0.0  # tiempo inicial

# parametros
sigma2_x = 0.01  # ruido proceso en posicion
...
for k in range(0, np.size(vals, 0)):
    ...
    Q = np.array([[sigma2_x, 0, 0, 0],
                  [0, 0.5, 0, 0],
                  [0, 0, sigma2_x, 0],
                  [0, 0, 0, 0.5]])  # ruido de proceso inicial
    R = np.array([[0.05, 0.0], [0.0, (np.deg2rad(1)) ** 2]])  # ruido de medicion inicial
    
    yk = H4D2D_cartesianToPolar(np.vstack([vals[k, 2], 0.0, vals[k, 3], 0.0]))  # obtener medicion
    xk0, PK0 = UKF_additiveNoise(xk0, PK0, uk, yk, Q, R, F4D4D_linearModel, H4D2D_identity)
    '''
    ...
    R = np.array([[sigma2_x, 0, 0, 0],
                  ...
                  [0, 0, sigma2_x, 0],
                  [0, 0, 0, 0.5]])  # ruido de proceso inicial
    yk = H4D4D_identity(np.vstack([vals[k, 2], 0.0, vals[k, 3], 0.0]))  # obtener medicion
